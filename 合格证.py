...
for i in range(pici):
	...
	while col <= (endnum - startnum + 1): #判断合格证生成的数量，数量未达到时无限循环
		...
		for i in range(3): #每一行生成三张合格证
			...
			worksheet.set_row(r_9, 24)
			worksheet.set_row(r_10, 15)
			...
			worksheet.write(chr(66 + c_0) + str(r_5), guige, format_text_u)
			worksheet.write(chr(66 + c_0) + str(r_6), qianzhui + sn, format_text_u)
			worksheet.write(chr(66 + c_0) + str(r_7), pihao, format_text_u)
			worksheet.write(chr(66 + c_0) + str(r_8), miejun, format_text_u)
			worksheet.write(chr(66 + c_0) + str(r_9), jianyanriqi, format_text_u)
			worksheet.write(chr(66 + c_0) + str(r_10), ' ', format_text_u)
