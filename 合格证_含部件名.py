...
xiangmu = ['产品名称:', '部件名称:', '规格型号:', '产品编号:', '生产批号:', '灭菌批号:', '检验日期:', '检验员号:']  #项目列表
...
for i in range(pici):
	...
	#生成工作表，用批号加序号作为工作表名
	worksheet = workbook.add_worksheet(pihao + '_' + str(i)) 
	...
	while col <= (endnum - startnum + 1): 
		...
		for i in range(3): 
			...
			#如果合格证数量已达到，跳出循环
			if col > (endnum - startnum + 1):  
				break
			
			#合格证上的序列号，默认为4位数字
			sn = str(int(startnum) + col - 1).zfill(4) 
			...
			#填写合格证信息
			worksheet.merge_range(chr(65 + c[0]) + str(r[1]) + ':' + chr(67 + c[0]) + str(r[1]),'公司名称',format_cop) #A1:C1
			...
			worksheet.write(chr(66 + c[0]) + str(r[5]), guige, format_text_u)
			worksheet.write(chr(66 + c[0]) + str(r[6]), qianzhui + sn, format_text_u)
			worksheet.write(chr(66 + c[0]) + str(r[7]), pihao, format_text_u)
			worksheet.write(chr(66 + c[0]) + str(r[8]), miejun, format_text_u)
			worksheet.write(chr(66 + c[0]) + str(r[9]), jianyanriqi, format_text_u)
